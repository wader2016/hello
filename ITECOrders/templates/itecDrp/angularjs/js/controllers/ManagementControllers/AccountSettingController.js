 ...
 */
MetronicApp.controller('AccountSettingController',['$scope','$http','authService','apiUrl','$timeout','$location','$rootScope','localStorageService', function ($scope, $http,authService,apiUrl,$timeout,$location,$rootScope,localStorageService) {
...
//----------------收货地址页面-------------------------------------
   ...
   $scope.setDefault = function() {
    ...
    $scope.deleteInfo= function () {
        ...
        for(var i=0;i<$scope.AddressInfo.length;i++){
            if(!$scope.AddressInfo[i].defaultSet){
                $scope.AddressInfo[0].defaultSet = true;
            }
        }
    };
    // 鼠标进入时 显示按钮
