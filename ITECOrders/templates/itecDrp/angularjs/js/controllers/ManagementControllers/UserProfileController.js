 ...
 */
MetronicApp.controller('UserProfileController',['$scope','$http','authService','$rootScope','localStorageService','apiUrl','$location', function ($scope,$http,authService,$rootScope,localStorageService,apiUrl,$location) {
  ...
  $http.get("/data/userData1.json").success(function (data) {
    ...
    $http.get("/data/Member1.json").success(function (data) {
        $scope.Member = data;
    });

    $scope.timeClick = function (time) {
        $http.get("/data/userData"+time+".json").success(function (data) {
            $scope.pageData = data.data1;
            $scope.dataLenth = 0;
            for(var i = 0;i<$scope.pageData.length;i++){
                $scope.dataLenth =  $scope.dataLenth + $scope.pageData[i];
        ...
        $http.get("/data/Member"+time+".json").success(function (data) {
            $scope.Member = data;
        });
