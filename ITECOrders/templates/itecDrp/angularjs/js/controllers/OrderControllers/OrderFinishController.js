...
// 获得数据
MetronicApp.controller('OrderFinishController',['$scope','$http','ModelService','apiUrl','$filter', function ($scope,$http,ModelService,apiUrl,$filter) {
    ModelService.getData(28).success(function(data){
        if(data.ErrMessage===null) {
            $scope.items = data.Data;
            for (var i = 0; i < $scope.items.length; i++) {
                $scope.items[i].isCheck = false;
                $scope.items[i].Actions = '通知备货';
            }
...
//-------------------数据处理部分----------------------------------------
    ...
    $scope.checkItems= function (item) {
        ...
        $http.get('http://'+apiUrl+'/Api/api/SalesOrder?state=28&catalog='+cateId).success(function (data) {
            $scope.items=data.Data;
            for(var i=0 ;i<$scope.items.length;i++){
                $scope.items[i].isCheck=false;
            }
    ...
    $scope.stuckConfirm=function() {
        ...
        if($scope.List.length>0){
            for(var i=0;i<$scope.List.length;i++){
                ...
                $scope.List[i].SalesOrderLineState='10';
                $scope.List[i].Actions='取消通知';
                $scope.testItem.push($scope.List[i].SalesOrderLineId);
            }
            $http.put('http://'+apiUrl+'/Api/api/SalesOrderLine?state=1',$scope.testItem).success(function () {
                alert('ok');
            })
        }
    };
    ...
    $scope.Confirm=function(){
        ...
        if($scope.list.Actions=='通知备货'){
            $scope.list.SalesOrderLineState='10';
            $scope.list.Actions='取消通知';
            $http.put('http://'+apiUrl+'/Api/api/SalesOrderLine?state=1', $scope.listId).success(function () {
                alert('ok');
            }).error(function () {
                alert('error');
            });
        }
        else if($scope.list.Actions=='取消通知'){
            $scope.list.SalesOrderLineState='0';
            $scope.list.Actions='通知备货';

            $http.put('http://'+apiUrl+'/Api/api/SalesOrderLine?state=0',$scope.listId).success(function () {
                alert('ok');
            }).error(function () {
                alert('error');
            });
