...
// 获得数据
MetronicApp.controller('OrderShippingController',['$scope','$http','ModelService','apiUrl','$filter', function ($scope,$http,ModelService,apiUrl,$filter) {
    ModelService.getData(26).success(function(data){
        if(data.ErrMessage===null) {
            $scope.items = data.Data;
            for (var i = 0; i < $scope.items.length; i++) {
                $scope.items[i].isCheck = false;
                $scope.items[i].Actions = '待运送';
            }
    ...
    ModelService.getArea().success(function (data) {
        if(data.ErrMessage==null){
            $scope.getStock = data.Data;
        }
    });
...
//-------------------数据处理部分----------------------------------------
    ...
    $scope.checkItems= function (item) {
        ...
        $http.get('http://'+apiUrl+'/Api/api/SalesOrder?state=26&catalog='+cateId).success(function (data) {
            $scope.items=data.Data;
            for(var i=0 ;i<$scope.items.length;i++){
                $scope.items[i].isCheck=false;
            }
    ...
    $scope.stuckConfirm=function() {
        ...
        if($scope.List.length>0){
            for(var i=0;i<$scope.List.length;i++){
                ...
                $scope.List[i].SalesOrderLineState='13';
                $scope.List[i].Actions='取消发运';
    ...
    $scope.Confirm=function(){
        ...
        if($scope.list.Actions=='待运送'){
            $scope.list.SalesOrderLineState='13';
            $scope.list.Actions='取消运送';
        ...
        else if($scope.list.Actions=='取消运送'){
            $scope.list.SalesOrderLineState='4';
            $scope.list.Actions='待运送';
